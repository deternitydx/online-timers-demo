...
var form_used = false;
var timer_type_changed = false;

var updateTimerInfo = function() {
    if (form_used) {
        form_used = false;
        timeallowed = $("#total-time").val();
        timeleft = timeallowed - $("#elapsed-time").val();
    }
    var percleft = ((timeleft / timeallowed)*100);
    if (!timer_started || timer_type_changed ) {
        timer_started = true;
        timer_type_changed = false;
        $('#timer').css('visibility', 'hidden');
        $('#timer').removeClass('progress');
        $('#timerbar').css('visibility', 'hidden');
        $('#timericon').css('visibility', 'hidden');
        $('#timertext').css('font-size', 'inherit').css('color', '');
        ...
        if (timer_method == 'bar-down') {
            ...
            $('#timerbar').css('visibility', 'visible');
        }else if (timer_method == 'moon' )
        {
            console.log(timeleft);

             //jQuery('<div>', {
              //   id: 'timericon',
               //  class: 'fas fa-moon fa-spin',
               //  style: 'font-size:30px; color: yellow; margin-left: auto; margin-right: 50%;'
            // }).appendTo('#timer');
    ...
    if (timer_method == 'bar-down' || timer_method == 'text-down' || timer_method == "stoplight" || timer_method == "grayshades" || timer_method == "hide-stoplight" || timer_method == "hide-grayshades-down" || timer_method == "green-down" || timer_method == "moon") {
            $('#timerbar').css('width', percleft+'%').attr('aria-valuenow',percleft);
            $('#timertext').text(timeleft + " minutes remaining");
            if(timer_method=='moon'){
            if(percleft>80){
           
                console.log(timeleft);
                $('#timericon').css('color', 'gray');
            }
            else if (percleft<=80&&percleft>60){
                
                console.log(timeleft);
            
            $('#timericon').css('color', 'green');
            }
            else if (percleft<=60&&percleft>40){
                
                console.log(timeleft);
            
            $('#timericon').css('color', 'yellow');
            }
            else if (percleft<=40&&percleft>20){
                
                console.log(timeleft);
            
            $('#timericon').css('color', 'blue');
            }
            else {
                
                console.log(timeleft);
            
            $('#timericon').css('color', 'red');
            }
        }

    } else if (timer_method == 'bar-up' || timer_method == 'text-up' || timer_method == "hide-grayshades-up" || timer_method=="moon") {
